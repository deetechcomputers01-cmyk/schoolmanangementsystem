...
export function PaymentActionsMenu({ studentId, feeRecordId }: PaymentActionsMenuProps) {
  const [open, setOpen] = useState(false);
  const editPaymentHref = `/fees?studentId=${studentId}${feeRecordId ? `&feeRecordId=${feeRecordId}` : ""}&recordPayment=1`;
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-44 overflow-hidden rounded-lg border border-line bg-white py-2 shadow-lg" role="menu">
          ...
          <Link className="flex items-center gap-2 px-4 py-2 text-xs font-semibold text-navy hover:bg-slate-100" href={editPaymentHref} role="menuitem">
